...
Refactor ore's architecture to eliminate the conflation of cognitive patterns (ReAct feedback loop) with IO orchestration (conduit event handling, rendering, status). `loop.Step` currently couples itself to `conduit.Conduit` for streaming delta rendering, while `orchestrate.ReAct` simultaneously owns both the ReAct feedback loop and the conduit event loop. This plan separates three distinct concerns: `loop.Step` becomes a conduit-agnostic transform node that emits output events; ReAct is extracted as a pure, stateless cognitive pattern under a new `cognitive/` package; and conduit/IO wiring becomes the sole responsibility of application-level code.

## Context

### Current Architecture (discovered from codebase)

- **`loop/loop.go`** — `loop.Step` struct embeds a `conduit.Conduit` field and calls `conduit.RenderDelta()` during streaming provider invocation. `WithSurface()` option configures this coupling. Step also runs `loop.Handler` artifact handlers (including tool execution) after each turn.
...
- **`orchestrate/orchestrator.go`** — `Orchestrator` interface (`Run(ctx) error`) bakes in conduit ownership by requiring the orchestrator to already own the conduit, state, provider, and step.
- **`orchestrate/react.go`** — `ReAct` struct embeds `conduit.Conduit`, `state.State`, `loop.Step`, and `provider.Provider`. It runs a conduit event loop in `Run()`, renders turns and status via the conduit, and delegates to `Step.Turn()` for inference. The actual ReAct feedback loop ("check state, loop if tool results exist") is thin; most of the code is IO wiring.
- **`examples/single-turn-cli/main.go`** — Uses `loop.New()` with no options. Output is entirely manual: the caller reads `mem.Turns()` after `Step.Turn()` returns. No conduit involved.
- **`examples/tui-chat/main.go`** — Passes the same TUI conduit `s` to both `loop.New(loop.WithSurface(s))` and `orchestrate.ReAct{Conduit: s}`. Conduit is split across two components.
- **`examples/calculator/main.go`** — Manually implements the ReAct feedback loop in `main()`: calls `Step.Turn()`, checks if last turn is `RoleAssistant`, loops if tool calls were handled. This is exactly the pattern `orchestrate.ReAct` should encapsulate — but without any conduit.
...
- **Step as transform node**: Step should define input/output formats and emit events to a subscriber-provided channel rather than calling `conduit.RenderDelta()` directly. This makes Step IO-aware (it knows output is happening) without being conduit-aware (it doesn't know the `Conduit` abstraction).
- **ReAct as pure cognitive pattern**: Stripped of conduit, state ownership, and event loops, ReAct becomes a stateless function: `Run(ctx, state.State) (state.State, error)`. It repeatedly calls `Step.Turn()` and inspects the resulting state.
- **IO wiring is application-level**: The conduit event loop, status management, interrupt handling, and rendering belong in `main()` (or a thin application-specific driver), not in framework packages.
...
**Before:**
```
loop/          → depends on conduit/ (Step calls RenderDelta)
orchestrate/   → ReAct owns conduit event loop + cognitive loop + rendering + status
examples/      → tui-chat passes conduit to both loop and orchestrate
```

**After:**
```
loop/          → no conduit dependency; emits OutputEvent to provided channel
cognitive/     → ReAct as pure feedback loop; no conduit, no state ownership
examples/      → application main() wires conduit events → state → cognitive.Run() → Step output → conduit rendering
```
...
2. **Cognitive layer (`cognitive.ReAct`)**: Stateless. Receives `state.State`, repeatedly invokes `Step.Turn()`, inspects resulting state to detect pending tool calls, loops until assistant turn is final. No conduit, no rendering, no event loop.
3. **IO wiring layer (application `main()`)**: Owns the `conduit.Conduit`. Reads `conduit.Events()`, appends user messages to state, calls `cognitive.ReAct.Run()`, subscribes to `Step.Output()` channel to route delta/turn events back to the conduit, manages interrupts and status.

## Requirements

1. `loop.Step` must not import `conduit/` or reference `conduit.Conduit`. Streaming deltas are emitted as `DeltaEvent` to a caller-provided `chan<- OutputEvent` instead of being routed to `conduit.RenderDelta()`.
2. `cognitive.ReAct` must be conduit-agnostic and stateless. It accepts `state.State` as a parameter and returns it, without embedding it. It must not reference `conduit.Conduit` or `conduit.Event`.
...
4. `examples/tui-chat/main.go` demonstrates the new architecture: conduit events are handled in `main()`, `cognitive.ReAct.Run()` is called for the cognitive loop, and `loop.Step` output events are routed to the TUI conduit via a goroutine.
5. `examples/calculator/main.go` uses `cognitive.ReAct` instead of manually implementing the feedback loop in `main()`.
6. `examples/single-turn-cli/main.go` requires no substantive changes (it already uses `loop.Step` without conduit).
...
- **Goal**: Introduce an `OutputEvent` event system into `loop.Step` so callers can subscribe to streaming deltas and turn completion without Step knowing about `conduit.Conduit`.
- **Dependencies**: None.
- **Files Affected**:
  - `loop/loop.go`
  - `loop/loop_test.go`
...
- **Interfaces**:
  ...
  - `WithSurface()` and `conduit` field are **preserved unchanged** in this task to maintain backward compatibility.
- **Validation**:
...
- **Details**: This is an additive change. Do not remove `WithSurface()` or the `conduit` field yet — `orchestrate/` still depends on them. Add new test functions (`TestStep_Turn_OutputEvents`, `TestStep_Turn_OutputEventsWithStreaming`) alongside existing conduit tests. The streaming goroutine logic is structurally identical to the current conduit goroutine; only the destination changes from `s.conduit.RenderDelta()` to `s.output <- DeltaEvent{}`.

### Task 2: Create cognitive/ Package with Pure ReAct Pattern
- **Goal**: Extract ReAct's cognitive feedback loop into a new `cognitive/` package, stripped of all conduit awareness and state ownership.
...
- **Interfaces**:
  ...
  - No conduit references. No `state.State` field embedding.
- **Validation**:
  - `go build ./...` passes.
...
- **Goal**: Restructure `examples/tui-chat/main.go` so all conduit IO wiring lives in `main()`, using `loop.WithOutput()` and `cognitive.ReAct`.
...
- **Details**: The new structure in `main()` should be:
  ...
  7. Event loop over `s.Events()`:
     - `conduit.UserMessageEvent`: append to state, call `s.SetStatus(ctx, "thinking...")`, call `react.Run(ctx, mem)`, call `s.SetStatus(ctx, "")`. Use a per-operation `context.WithCancel(ctx)` so `conduit.InterruptEvent` can cancel only the in-progress `react.Run`, not the entire application.
     - `conduit.InterruptEvent`: cancel the current operation's context.
...
### Task 6: Remove conduit Dependency from loop/
- **Goal**: Strip the remaining conduit awareness from `loop.Step` by removing `WithSurface()`, the `conduit` field, and the `conduit/` import.
...
- **New Files**: None.
- **Interfaces**:
  - Remove `WithSurface(surf conduit.Conduit) Option`.
  - Remove `conduit` field from `Step` struct.
  - Remove `conduit` import from `loop/loop.go`.
  - Update `Turn()`: remove the code path that calls `s.conduit.RenderDelta()`. The output channel path (added in Task 1) is the sole mechanism for streaming output.
- **Validation**:
  - `go build ./...` passes with no errors.
  - `go test -race ./...` passes.
  - `grep -n "conduit" /home/andrewhowdencom/Development/ore/loop/loop.go` should find no matches.
  - `grep -n "conduit" /home/andrewhowdencom/Development/ore/loop/loop_test.go` should find no matches.
- **Details**: Update `loop/loop_test.go` to remove all `mockSurface` code and tests that depend on `WithSurface()`. The output-event tests added in Task 1 remain and now cover the sole streaming mechanism. Update `loop/doc.go` to remove references to "optionally routes streaming deltas to a conduit" — replace with "optionally emits streaming deltas as OutputEvent to a provided channel".
...
| Output channel goroutine leaks or races during streaming | Medium | Medium | Ensure `close(deltasCh)` and `wg.Wait()` are preserved from the old conduit goroutine pattern. Use buffered channels (size 100) and `select` with `ctx.Done()`. Race tests in `go test -race` will catch issues. |
...
| `loop_test.go` conduit tests deleted prematurely, losing coverage | Medium | Low | Task 1 adds output-event tests *before* Task 6 removes conduit tests. There is always test coverage for streaming behavior. Verify `go test -race ./loop/...` passes after each task. |
...
- [ ] `loop/` package contains no imports of `conduit/` and no references to `conduit.Conduit`.
...
- [ ] `cognitive.ReAct` struct does not contain `conduit.Conduit` or `state.State` fields.
- [ ] `examples/tui-chat/main.go` compiles and demonstrates the three-layer architecture (conduit in `main()`, `cognitive.ReAct` for loop, `loop.WithOutput` for streaming).
